...
export default function RentalsPage() {
  ...
  const getStatusBadge = (status: Rental["status"]) => {
    const config = {
      ...
      Returned: "bg-emerald-50 text-emerald-700 border-emerald-200/60 ring-emerald-500/10",
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold tracking-wide rounded-full border shadow-sm ${config[status]}`}>
        <span className={`h-1.5 w-1.5 rounded-full current-color ${
          status === "Borrowed" ? "bg-blue-500" : status === "Returned" ? "bg-emerald-500" : "bg-rose-500"
        }`} />
        {status}
      </span>
    );
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      {rentals.length === 0 ? (
        <div className="text-center py-16 px-4 bg-white border border-slate-200/60 rounded-2xl shadow-sm max-w-xl mx-auto mt-8">
          ...
          <h3 className="text-base font-bold text-slate-900 tracking-tight">No borrowed books</h3>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-3 text-xs font-bold text-slate-400 uppercase tracking-wider">
                    <span className="flex items-center gap-1">
                      ...
                      <span className="text-slate-600 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">{rental.book.category}</span>
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="text-slate-500">ISBN:</span>
                      <span className="text-slate-600">{rental.book.isbn}</span>
                    </span>
                  </div>
                ...
                <div className="mt-5 pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
                  <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-slate-600 font-semibold">
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-400 font-bold uppercase tracking-wider">Borrowed:</span>
                      <span className="text-slate-800">{new Date(rental.rentalDate).toLocaleDateString()}</span>
                    </div>
                    
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-400 font-bold uppercase tracking-wider">Due Date:</span>
                      <span className={`font-bold ${rental.status === "Overdue" ? "text-rose-600" : "text-slate-800"}`}>
                    ...
                    {rental.returnDate && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-slate-400 font-bold uppercase tracking-wider">Returned:</span>
                        <span className="text-emerald-600">{new Date(rental.returnDate).toLocaleDateString()}</span>
                      </div>
                    )}
                  </div>

                  {/* Return Action Placeholder Button Zone */}
                  {rental.status !== "Returned" && (
                    <button 
                      onClick={() => console.log("Init return process via payload id:", rental._id)}
                      className="text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100/80 border border-blue-100 rounded-xl px-4 py-2 transition-all duration-150"
                    >
                      Request Return
                    </button>
                  )}
